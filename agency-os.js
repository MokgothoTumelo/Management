...
/* ============================= STORAGE (per-account via auth.js) ============================= */
/*
 * All app data is scoped to the logged-in user through auth.js:
 *   getUserData(key) / saveUserData(key, value)
 * Keys become: agency-os:user:{userId}:clients | templates
 * This isolates User A from User B on the same browser.
 * Later: swap these helpers for Firestore reads/writes.
 */
...
async function loadState() {
  ...
  // Clients — per account
  try {
    const data = typeof getUserData === 'function' ? getUserData('clients') : null;
    if (Array.isArray(data) && data.length >= 0 && data !== null) {
      // Empty array is valid (new account with no clients yet)
      STATE.clients = Array.isArray(data) ? data : seedClients();
    } else if (data === null) {
      // First visit for this account: seed demo clients once
  ...
  try {
    const data = typeof getUserData === 'function' ? getUserData('templates') : null;
    if (data === null) {
...
function saveClients() {
  if (typeof saveUserData !== 'function' || !saveUserData('clients', STATE.clients)) {
    toast('Could not save — changes may not persist');
  }
}

function saveTemplates() {
  if (typeof saveUserData !== 'function' || !saveUserData('templates', STATE.templates)) {
...
function pageFor(route) {
  return ROUTE_PAGES[route] || 'dashboard.html';
}
...
function goto(route, extra) {
  ...
  if (current === target || (current === '' && target === 'dashboard.html')) {
    render();
    window.scrollTo(0, 0);
    return;
  }
